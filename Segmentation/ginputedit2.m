function [out1,out2,out3] = ginputedit2(arg1)
...
thickness = gap; 
size = 25;
set(crossHair(1), 'Position', [cp(1)-size cp(2) size thickness]);
set(crossHair(2), 'Position', [cp(1)+gap cp(2) size thickness]);
set(crossHair(3), 'Position', [cp(1) cp(2)-size thickness size]);
set(crossHair(4), 'Position', [cp(1) cp(2)+gap thickness size]);
end
...
for k = 1:4
    crossHair(k) = uicontrol(fig, 'Style', 'text', 'Visible', 'off', 'Units', 'pixels', 'ForegroundColor', [0 1 0],'BackgroundColor',[0 1 0], 'HandleVisibility', 'off', 'HitTest', 'off'); %#ok<AGROW>
end
end
