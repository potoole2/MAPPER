function testnew(M,N,tempDirectory,channels,numembs)
...
overlaps1 = zeros(M*N,length(channels),100,100,overlapsize);
overlaps2 = zeros(M*N,length(channels),100,100,overlapsize);
for i= 1:M*N
    for k = 1:length(overlaps(i).reg)
        ...
        for h = 1:length(channels)
            overlaps1(i,h,:,:,k) = imresize(double(tiles{i,1}(i1:i2,j1:j2,h)),[100 100],'bicubic');
        end
    end
end
% for i=1:M*N
...
x = fminsearch(@testfunctionnew,x0,optimset('MaxFunEvals',10),overlaps,overlaps1,chan);

for i = 1:M*N
    for j = 1:length(channels)
        tiles{i,1}(:,:,j) = tiles{i,1}(:,:,j).*x(j,i) + x(j,i + M*N);
        if j == 1
